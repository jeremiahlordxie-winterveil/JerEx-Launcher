...
> **告别 Windows ChatGPT / Codex 客户端频繁 `Reconnecting...` 的困扰。**  
> 一键启动 · 智能代理探测 · 自动环境注入 · 丝滑直连
...
1. **频繁卡在 `Reconnecting...`**：由于网络或代理节点握手异常，客户端无法稳定建立连接。
2. **代理端口变动后失效**：代理软件（如 Clash、v2rayN、Mihomo）切换了端口或协议后，ChatGPT 仍在读取旧配置导致离线。
3. **手动修改 `.env` 繁琐且易错**：每次都要去寻找 `%USERPROFILE%\.codex\.env` 手动填入 `HTTP_PROXY`，容易改错或覆盖其他配置。

**JEL (JerEx Launcher)** 为此而生！它在客户端启动前自动进行三级网络体检，智能选出可用的本地代理并安全注入配置，保障每一次打开都能稳定直连。

---

## ✨ 核心特性

- ⚡ **智能代理发现**：内置常见代理端口池（7897, 7890, 10808, 10809, 8080 等），支持 HTTP / SOCKS5 自动识别，优先复用上次有效配置，实现秒开。
- 🔍 **三级深度连通性验证**：
  - [x] 基础外网连通测试（`example.com`）
  - [x] OpenAI 核心 API 连通测试（`api.openai.com`）
  - [x] ChatGPT 前端网关握手（`chatgpt.com`）
- 🛡️ **无损注入与安全备份**：仅安全更新代理相关环境变量，完整保留用户的其他配置项；更新前自动生成带时间戳的历史备份，杜绝配置损坏。
- 🔄 **进程智能协同**：如果代理配置发生变更且客户端正在运行，会弹出友好提示，由用户确认是否平滑重启以使新配置生效。
- 📦 **绿色免装，随处运行**：**不需要**放到特定的 GPT 目录下，放在电脑任意位置均可自动寻址并拉起应用。
